...
$arrTag['title'] = $tag['SEO_title'];
$arrTag['description'] = $tag['SEO_description'];
$arrTag['keyword'] = $tag['SEO_keyword'];
$arrTag['category'] = $tag['name'];
...
FunctionPub::dq_menu_childrent($all_menu, 0, 'id_category', $cateCon, '');

$query = Article::find()
    ->distinct()
    ->asArray()
    ->joinWith('tag_vs_post')
    ->joinWith('category')
    ->where(['tag_vs_post.id_tag' => $idTag, 'article.status' => 1])
    ->select([
        'article.*',
        'tag_vs_post.id_tag',
    ]);
                ...
                <nav aria-label="breadcrumb">
                    <ol class="breadcrumb">
                        ...
                        <li>
                            ...
                            <a href="/tag/<?= $tag['link'] ?>-<?= $tag['id_tag'] ?>"
