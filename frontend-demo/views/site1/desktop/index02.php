<?php
/**
 * Created by PhpStorm.
 * User: User
 * Date: 5/16/2019
 * Time: 11:17 AM
 */
?>
<div class="kt-index-left-box"
     style="background-image: url(/images/home-bg.png)">
    <div style="min-height: 100px"></div>

    <h1>Cộng đồng<br>kiến trúc sư Việt Nam.</h1>
    <div class="kt-index-text">
        Nơi tập hợp tất cả kiến trúc sư, nhà thiết kế nội thất, kỹ sư xây dựng xuất sắc nhất Việt Nam.
        Liên hệ tư vấn thiết kế , giám sát thi công với bất kì chuyên gia nào.
    </div>
    <div class="kt-index-slogan">
        <b>TRỰC TIẾP - NHANH CHÓNG - ĐƠN GIẢN</b>
        chỉ với 1 click chuột.
    </div>
    <ul class="list-unstyled">
        <li>
            <b>435</b>
            <span>Kiến trúc sư</span>
        </li>
        <li>
            <b>1.288</b>
            <span>Dự án kiến trúc</span>
        </li>
        <li>
            <b>1.240</b>
            <span>Thành viên</span>
        </li>
        <li>
            <b>10.304</b>
            <span>Hình ảnh công trình</span>
        </li>
    </ul>
    <div class="clearfix"></div>
    <div style="min-height: 15px"></div>
    <a href="" title="">
        <button type="button" class="btn kt-index-default">
            Đăng ký miễn phí ngay
        </button>
    </a>

</div>
